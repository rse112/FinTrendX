...
def execute_analysis(results,month_rule_list,rising_list,select_list):
    with ProcessPoolExecutor() as executor:
        future_to_function = {
            executor.submit(monthly_rule, keyword_data, today_date, 'month'): ('monthly', keyword_data)
            for keyword_group in results
            for keyword_data in keyword_group
        } | {
            executor.submit(rising_keyword_analysis, keyword_df, today_date, period): ('rising', keyword_df, period)
            for keyword_df_group in results
            for keyword_df in keyword_df_group
            for period in ['weekly', 'month']
        } | {
            executor.submit(select_keyword, keyword_df, today_date, period): ('select', keyword_df, period)
            for keyword_df_group in results
            for keyword_df in keyword_df_group
