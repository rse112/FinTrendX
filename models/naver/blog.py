...
def activity_rate(keywords_path):
    ...
    for keyword in tqdm(keywords, desc="Processing Keywords"):
        ...
        activity_rate = blog_result([keyword], today)  # blog_result 함수는 리스트를 입력으로 받으므로, keyword를 리스트로 전달
        activity_rates.append(activity_rate[0])  # 반환값이 리스트 형태이므로, 첫 번째 요소만 추가

    # 결과 데이터프레임 생성
    df_results = pd.DataFrame({
        "Keyword": keywords,
        "Activity Rate": activity_rates
    })
    # 결과 출력


    # CSV 파일로 저장
    df_results.to_csv(f"./data/target_keywords/{formatted_today}/keyword_activity_rates.csv", index=False)
